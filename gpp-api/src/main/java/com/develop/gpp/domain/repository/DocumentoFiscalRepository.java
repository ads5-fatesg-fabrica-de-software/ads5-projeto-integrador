...
public interface DocumentoFiscalRepository extends JpaRepository<DocumentoFiscalModel, Integer> {
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM documento_fiscal df INNER JOIN item_documento_fiscal idf ON idf.item_doc_id = df.id_documento_fiscal INNER JOIN produto p ON idf.id_produto = p.id_produto WHERE p.id_produto = :prod")
